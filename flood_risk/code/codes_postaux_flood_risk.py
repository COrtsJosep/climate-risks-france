...
gdf_cp = gpd.read_file(geoshapes_dir / 'codes_postaux_geoshapes.geojson').to_crs('EPSG:4326')

### 4. CALCULATIONS
gdf_cp.loc[:, 'prop_at_flood_risk'] = gdf_cp.loc[:, 'geometry'].apply(lambda cp_shape: cp_shape.intersection(fr_shape).area / cp_shape.area)
